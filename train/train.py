...
def train_model(model, X_train, Y_train, X_test, Y_test, device, epochs=30, batch_size=16, lr=0.0001, gamma_rec = 0.5, checkpoint_path='/root/autodl-tmp/drw-prediction/checkpoints/'):
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in train_loader:
            ...
            yb_AE = xb # torch.cat([xb[:, :5], xb[:, -58:]], dim=1)
            pred_enc, pred_dec, pred_out = model(xb)
            loss = loss_fn(pred_out, yb) + gamma_rec * loss_fn(pred_dec, yb_AE)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for xb, yb in test_loader:
                ...
                yb_AE = xb # torch.cat([xb[:, :5], xb[:, -58:]], dim=1)
                pred_enc, pred_dec, pred_out = model(xb)
                test_loss += (loss_fn(pred_out, yb).item() + gamma_rec * loss_fn(pred_dec, yb_AE).item())
        ...
        if avg_test_loss < best_test_loss :
            ...
            if checkpoint_path : # and epoch >= 49:
                filename = f"model_epoch{epoch+1:02d}_test_loss{avg_test_loss:.4f}.pt"
                full_path = os.path.join(os.path.dirname(checkpoint_path), filename)
                torch.save(model.state_dict(), full_path)
                print(f"Saved best model to {full_path}")
                cleanup_checkpoints(checkpoint_path, max_keep=20)

def train_model_w_ml(model, X_train, Y_train, X_test, Y_test, device, epochs=30, batch_size=16, lr=0.0001, checkpoint_path='/root/autodl-tmp/drw-prediction/checkpoints/'):
    train_ds = TensorDataset(torch.tensor(X_train, dtype=torch.float32), torch.tensor(Y_train.values, dtype=torch.float32).view(-1, 1))
    test_ds = TensorDataset(torch.tensor(X_test, dtype=torch.float32), torch.tensor(Y_test.values, dtype=torch.float32).view(-1, 1))
    ...
    for epoch in range(epochs):
        ...
        y_train_ml = np.vstack(list_y_train).ravel()  
...
def cleanup_checkpoints(folder_path, max_keep=20, file_patterns=('*.pt', '*.pkl')):
    files = []
    ...
    # 删除最旧的文件，直到只剩 max_keep 个
    files_to_delete = files[:len(files) - max_keep]
