...
# —— 模型定义 ——————————————————————————
tabnet_params = dict(
    n_d=64, n_a=64, n_steps=16, gamma=1.5,
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X), 1):
    ...
    corr = pearsonr(y_scaler.inverse_transform(preds_val.reshape(-1,1)).ravel(),
                    y_scaler.inverse_transform(y_val.reshape(-1,1)).ravel())[0]
    print(f"Fold {fold} valid Pearson r: {corr:.4f}")
...
# —— 最终训练 + 验证提交  ——————————
model_final = TabNetRegressor(**tabnet_params)
model_final.fit(X, y.reshape(-1, 1), eval_set=[(X_val, y_val.reshape(-1, 1))], max_epochs=300, patience=50,
               batch_size=8192, virtual_batch_size=1024, num_workers=8, drop_last=False )
pred_test = model_final.predict(X_test)
submission["prediction"] = y_scaler.inverse_transform(pred_test.reshape(-1,1)).ravel()
submission.to_csv("submission_tabnet2.csv", index=False)
